...
\begin{enumerate}
\item This lifts the structure of quantum computation into the general setting of operational generalised compositional theories. This allows the study of protocols like teleportation, state transfer, quantum secret sharing, quantum bit commitment, Mermin non-locality tests, and other protocols to be handled at an abstract level and studied in different oGCTs.
\item The categorical diagrams that accompany oGCT's present a more powerful quantum circuit language. This is one that includes bases, classical information, complementarity, and phases explicitly and posses rules for manipulating these structures within the diagrams themselves. We give a summarizing table of these structures at the end of this chapter.
\end{enumerate}
Both the abstraction and diagrammatic formalism provide powerful tools throughout the results presented here.

%\newpage

\begin{figure}[b]
\caption{A summary of the diagrammatic elements that oGCTs have above quantum circuit diagrams.}
\begin{tabulary}{\linewidth}{|p{5cm}|c|}\hline
The Dagger $\dagger:\cat{C}\to\cat{C}$ generalizes the adjoint and vertically flips the whole diagram.
& \input{tikz/1_daggerPics.tikz} \\\hline
Scalars $s:I\to I$. These float freely in diagrams
& \begin{tikzpicture}[xscale={\tikzxscale}, yscale={\tikzyscale}]
\node [whitedot] (0) at (-2, -0) {$s$};
...
Classical structures (i.e. generalized observables) & \input{tikz/2_spiderdef.tikz} \\\hline
