...
\begin{defn}
\label{def:dimension}
In an oPT, the \textbf{dimension} $d(A)$ of an object $A$ equipped with a dagger-Frobenius algebra \whitefrob{A} is given by the following composite:
\begin{equation}
\label{eq:dim}
...
In an oPT, two classical structures (\dotonly{whitedot}) and (\dotonly{graydot}) on the same object are \textbf{complementary} when the following equation holds:
\begin{equation}
...
In an oPT, two classical structures (\dotonly{whitedot}) and (\dotonly{graydot}) are \textbf{coherent} when the following equations hold:
\begin{equation}
\label{eq:coherence}
...
\begin{example}
\label{ex:cnot}
Complementary observables allow us to construct a generalized form of the controlled-not gate in any oPT. In \cat{FHilb} we can choose classical structures on the two-dimensional Hilbert space that correspond with the usual Z and X observables:  \begin{align*}
    \tinycomult[whitedot] : &
      \begin{array}{rcl}
        \ket{0} & \mapsto & \ket{00} \\
        \ket{1} & \mapsto & \ket{11}
...
Some oPTs, such as \cat{FHilb}, come equipped with linear structure on their processes. Abstractly these are enriched categories, i.e. categories where hom-sets are replaced with other objects.\footnote{Kelly~\cite{kelly1982basic} can be used as a standard reference for enriched category theory.}
