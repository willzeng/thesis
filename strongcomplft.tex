...
\begin{equation} \label{eqn:HadamardMatrixDef}
  \mathbf{F}_{hg} = \exp\left[\, i \, \sum_j \frac{2 \pi}{n_j} \left(\modclass{g_j-h_j}{n_j}\right) \, \right] %\text{ for } h = (h_j)_{j=1,...,D} \text{ and } g = (g_j)_{j=1,...,D} \text{ in } G = \prod_{j=1}^D \integersMod{n_j}
\end{equation}
...
\begin{remark}
\todo{Note that the antipode is self-inverse when the structures... }
\end{remark}
