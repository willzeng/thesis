...
\begin{defn}
Let $\ket{x}_{x \in X}$ be a finite family of states $I \rightarrow \SpaceG$ in a $\dagger$-SMC which is distributively $\cat{fCommMon}$-enriched. A \textbf{matchable family} $\ket{x}_{x \in X}$ for a monoid $(\SpaceG,\XmultSym, \XunitSym)$ are those for which the following holds for all $x,y \in X$:
...
\end{proof}
Note that, by Definition \ref{def:Antipode} and Frobenius law for $\XdotSym$, Equation \ref{eqn:orthogonalityMultChars} can equivalently be written as the following, stating that the multiplicative characters are a matchable family for $(\tinycomult[whitedot],\tinycounit[whitedot])$: 
\begin{equation}
