...
When we program an abstract problem into an oGCT, we have a choice of embeddings, but typically assign classical information to the classical states. This was the case in our treatment of the Fourier transform for example. Of course, we will also want to consider how to program functions between those classical states.

\begin{corollary}
In \cat{FHilb}, a morphism between classical structures acts as a function on classical states if and only if it is a self-conjugate comonoid homomorphism.
\end{corollary}
In \cat{FHilb}, this says that functions mapping from one basis to another will necessarily be comonoid homomorphisms.  This idea and its generalizations form the core of oracles in quantum and oGCT computation.
...
The oracle that we will build it actually similar to the CNOT from Section \todo{REFERENCE}. Recall that a pair of symmetric dagger-Frobenius algebras can be used to build a linear map in the following way:
\begin{equation}
\sqrt{\ud(A)}\,\,
...
Directly mapping the (rotated) quantum circuit for the DJ algorithm into an oGCT process gives:
\begin{align}
\begin{aligned}
...
Simplifying using the Fourier Transform rules we obtain:
\begin{align}
...
\;= \;   \begin{cases}
    ...
    \end{cases}
\end{equation}
Thus, by the oGCT Born rule, there is no probability that an outcome $\sigma$ would be measured for a balanced function.  Conversely, should the coefficients of $\sigma$ be square roots of the identity, then $\sigma$ will always be measured for constant $f$.  This allows us to deterministically distinguish large generalized classes of constant and balanced $f$ with a single oracle query.  

In a single proof, this method and its extension to arbitrary finite groups~\cite{vicary-tqa} captures the original formulation of the problem as well as the generalizations of~\cite{hoyer1999conjugated} and~\cite{batty2006extending}. Further, we can now model the algorithm in oGCT's besides quantum theory that has strongly complementary observables. We expand on this idea in Section~\ref{sec:qalgrel} with a toy model in $\cat{Rel}$. 

\subsection{\todo{The Grover's and hidden subgroup algorithms}}
Through similar methods, the single-shot Grover's and hidden subgroup algorithms can be analyzed using categorical techniques~\cite{vicary-tqa}. These algorithms have the diagrammatic forms given in Figure~\ref{fig:algtriplet}. The Grover's generalization produces a variant where the indicator function maps into a group, rather than a set. \todo{comment on connection to amplitude amplification}
